...
**Read `STANDING_DIRECTIVE.md` at the start of every session; it outranks older instructions on conflict** (it sits *below* the Constitution, alongside the operational directives below).

**Operational directives — read `EXECUTIVE_DIRECTIVE.md` FIRST every session.** It governs positioning, launch, revenue, and marketing, and **outranks all older operational instructions on conflict** (it sits *below* the Constitution, not above it). **Exactly two active directives:** `EXECUTIVE_DIRECTIVE.md` (positioning/launch/revenue/marketing/Misk) › `MASTER_DIRECTIVE.md` (phases/gates/standing rules). Everything in **`docs/archive/`** — including `PHASE2_MATCHING.md` and `PHASE2_REVISED.md` — is **reference only and must NEVER be executed from** (their matching-is-the-bottleneck thesis was disproven by ADR-133).
...
- **Never run a process-kill command (`taskkill`, `pkill`, `kill`, etc.) against a process you did not yourself start this session.** Incident (2026-08-22): a stray `taskkill /IM chrome.exe` — intended as a scratch/placeholder command — killed every Chrome process on the machine, not just the automation tab, closing whatever the user had open. Before killing anything, verify the PID is one this session launched (e.g. captured from its own `nohup .../background` invocation); killing by image name (`/IM`) or any other blast-radius-widening match is exactly the failure mode that caused the incident and must never be used.
...
**Phase 2 — Decision Intelligence & Provider Platform (durable rules).**
- **Trust is one thing, computed one way.** `src/lib/intelligence/evidence-engine.ts` (`assessTrust`/`productTrust`) is the single, deterministic, evidence-**cited** trust source used by decide, tps/search, tps/recommendations, and the UCP feed. It COMPOSES existing primitives (price verdict, store trust, discount integrity, corroboration) — never re-derive them, never reintroduce ad-hoc confidence heuristics. Corroboration dominates; a missing signal lowers trust and surfaces a caveat (never inflates). No LLM in the score/ranking path (ADR-002).
- **Merchants onboard through the Provider Framework** (`src/lib/providers/`), never one-off. A `RetailerProvider` binds SOURCING (scraper | official_feed | affiliate_feed | api | csv_xml — all yield `ScrapedProduct[]`, TPS untouched) × MONETIZATION (pluggable `AffiliateNetwork`). `/go` monetizes exits via `buildOfferExitLink()` only. New affiliate program / feed = configuration + an adapter, not new exit logic. Every connector must strengthen the framework.
- **Feed sourcing facts** (ADR-089): shaker's DEFAULT sourcing is the WooCommerce Store API (`sourcing: "api"` + `feedUrl`), not the scraper — cleaner and more complete. `provider.sourcing` is read ONLY by the sourcing adapters/ingest path, never by `/go` or a request route. Multilingual (WPML/Polylang) Woo/Salla shops serve each product once per language sharing ONE SKU; the feed adapter MUST dedup EN/AR pairs by SKU (`mergeMultilingualBySku`, Arabic-first) or `raw_observations` double-counts (the normalizer keys on URL, not SKU). Feed-sourced stores live in the scheduler's `INGEST_FEED_STORES` loop and are EXCLUDED from the scraper `INGEST_STORES` set so no store ingests twice; both are env-reversible.
- **Comparison growth is merchant-DATA-ACCESS-bound, not engineering-bound.** ~89% of products are single-store; high-overlap Saudi retailers block scraping. More parsers/identity/one merchant ≈ 0 new comparisons. The real lever is overlapping-SKU merchant *data* (WooCommerce Store API is the credential-free path; affiliate/official feeds + Salla OAuth + PA-API are Founder-credential boundaries the framework plugs in config-only). `tps:comparison-value`'s "comparison possible" = multi-merchant BRAND, not multi-store LISTING.
- **Automation runtime facts** (ADR-078): Railway routes traffic to the START command's MAIN process; Supabase's direct host is IPv6-only while Railway is IPv4 → route `SUPABASE_DB_URL` through `scripts/tps-core/pooler-url.js`; `node --check` does not catch bad `require` paths; production guards must accept the pooler URL. Internal identity sentinels (NO_STORAGE/NO_TECH/NO_SERIES/NA) must be stripped at EVERY customer render path.
- **Pipeline concurrency (ADR-099, learned the hard way):** NEVER run heavy pipeline writers (`normalize`, `build-tps-projection`, `refresh-intelligence`) concurrently — with each other OR the hourly scheduler. The scheduler OWNS realization; manual runs must be serialized, `--dry`-first, and never parallelized — concurrent heavy jobs overload the DB, bloat the system catalogs, and wedge PostgREST in the `PGRST002` schema-cache loop (the REST-backed customer endpoints go dark). If a PostgREST restart does NOT clear `PGRST002`, the authenticator role's `statement_timeout` is too low for cold catalog introspection — raise it (prod is at 30s) and `VACUUM (ANALYZE)` the catalogs; a bare `NOTIFY pgrst 'reload'` re-introspects WITHOUT reconnecting and just re-breaks it, only a full project restart reconnects cleanly. Current relaxed role timeouts (kept to prevent recurrence): authenticator 30s, anon/authenticated/service_role 20s.
- **PostgREST silently truncates at `db-max-rows` (ADR-172, recurred and generalized at ADR-285):** a bare `.limit(n)` on a Supabase/PostgREST query is capped by the project's `db-max-rows` setting (currently **1000**) regardless of the requested `n`, and returns `error: null` — there is no signal that the response was truncated. Any query whose result feeds a founder-facing number, a customer-facing count/list that must be complete, or any other COUNT a human will quote **MUST** use `fetchAllPaginated()` (`src/lib/database/paginated-fetch.ts`) instead — never a bare `.limit()` for an "all matching rows" fetch. The caller must add a deterministic `.order()` (a primary key or another unique/stable column) before paginating — an unordered `.range()` loop is exactly as unstable as an unordered `.limit()`. This does NOT apply to a `.limit()` that is itself the intended page size for a small, deliberately-bounded UI list (an autocomplete dropdown, a "recent N" feed) — the risk is specifically an unbounded "give me everything" query dressed up as a large `.limit()`. A `count: 'exact', head: true` query is unaffected by this cap (it returns a true `SELECT COUNT(*)`, not a row slice) and needs no pagination. Run `npm run tps:postgrest-cap-check` to re-verify the known-fixed and known-watched queries against live production on demand. **Any review of a query touching founder-dashboard or analytics tables (ADR-278's style of review included) must explicitly check for this pattern, not only business-logic correctness** — ADR-278 reviewed `command-center-queries.ts` line-by-line four days before ADR-285 found this exact defect in it, because that review checked funnel semantics, not PostgREST infrastructure limits.
