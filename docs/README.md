...
| فحص رابط منتج (Check) | `src/lib/check/`؛ `src/components/check/`؛ `/[locale]/check`؛ `/api/check` | `tests/check/`؛ [ملاحظة الإغلاق الموحّدة](report/TAWVEERI-CHECK-CLOSEOUT-2026-09-17.md) تلخّص كل تقارير/ADR-367 حتى 374 | **مجمّدة ومخفية عن التنقل الظاهر منذ ADR-374 (2026-09-17)** — الكود والبيانات سليمة، الصفحة تعمل بالرابط المباشر فقط. لا تُعاد أو تُوسَّع دون استخدام حقيقي مثبت أولاً |
