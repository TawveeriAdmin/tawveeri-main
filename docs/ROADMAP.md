...
| **E14** | Owned search index authority cutover | ⛔ **Gated on catalog coverage (not a secret):** owned TPS index = **48 of 4,821 catalog products (~1%)**; TPS plugins exist for **mobile+ac only**. Making it sole authority would collapse search to 48 items. Engineering (owned index E5, /api/v1 consuming it) is done; the CUTOVER needs TPS identity contracts for all categories + canonical scaling (132k observations, 157 processed). Canonical Smart-Pick overlay already live for mobile+ac. |
