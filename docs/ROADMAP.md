...
| **E6** | TPS pipeline automation + **canonical-quality audit** (resolves the identity-invariant debt) | **Audit + taxonomy remediation SHIPPED & PRODUCTION-VERIFIED (ADR-011/013/014)** — 731 canonical rows corrected (ACs→air_conditioner, fridges/washers→appliance, enum de-dup); `accessories` 1079→387. **Pipeline automation BLOCKED (ADR-014)** — needs `SUPABASE_DB_URL` + pipeline specs, or founder approval, to run a bounded ingestion batch over 129,715 pending observations. |
