...
| **E11** | Mobile convergence — true platform client, measured exits | 🚧 **In progress (ADR-029, `docs/HANDOFF-E11.md`)** — measured-exit groundwork shipped (`/go?source=` + mobile `openMeasuredExit` wired at all 4 exit surfaces). **Full convergence gated:** needs the platform-contract change (offerIds → mobile), 45 catalog-read replacements, E10 (🔒 legacy creds), and an app-store release (external review). **XL.** |
