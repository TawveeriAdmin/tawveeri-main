...
| **E14** | Owned search index authority cutover | ⛔ **Gated on catalog coverage (not a secret):** owned TPS index = **74** corroborated canonicals across 6 live categories (the corroboration ceiling of the current 4-store catalog). Coverage is gated by **cross-store corroboration, not plugin count** — the laptop plugin is built + precise yet the Saudi laptop catalog has **0 genuine ≥2-store matches** (ADR-032). Corroboration ceilings: **all corroborating categories shipped (TV/Tablet/Audio/Camera✅); Appliance=0 deferred** (ADR-034/035/037/038). **Category sweep complete → E14 is now a HYBRID design** (canonical comparison for the ~74 corroborated + labelled single-store discovery for the rest), not a sole-index cutover; repeated bounded batches grow the corroborated set. Smart-Pick overlay live for all 6 categories. See **§6 TPS Category Coverage**. |
