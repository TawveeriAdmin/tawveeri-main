...
| **E14** | Owned search index authority cutover | ⛔ **Gated on catalog coverage (not a secret):** owned TPS index = **94** corroborated canonicals (full-catalog saturation via progressive batching, ADR-039); identity resolved for **812 products** (756 single-store). Coverage is gated by **cross-store corroboration, not plugin count** — the laptop plugin is built + precise yet the Saudi laptop catalog has **0 genuine ≥2-store matches** (ADR-032). Corroboration ceilings: **all corroborating categories shipped (TV/Tablet/Audio/Camera✅); Appliance=0 deferred** (ADR-034/035/037/038). **Progressive batching SHIPPED + saturated (94 corroborated, 812 resolved) → E14 HYBRID ready to implement** (canonical comparison for the ~74 corroborated + labelled single-store discovery for the rest), not a sole-index cutover; repeated bounded batches grow the corroborated set. Smart-Pick overlay live for all 6 categories. See **§6 TPS Category Coverage**. |
