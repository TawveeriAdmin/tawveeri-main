...
| **E6** | TPS pipeline automation + **canonical-quality audit** (resolves the identity-invariant debt) | **Mobile pipeline SHIPPED & PRODUCTION-VERIFIED, bounded & repeatable (ADR-011/013/014/015/016/017/018/019 + runbook).** 788 canonical taxonomy fixes; bounded matcher (≤500 obs, dry-run-first, atomic, idempotent, rollback); projection rebuilt (3→41, iPhone 15 live); `mobile`/`smartphone` resolved via the two-plane model (fixed an ADR-014 regression that hid 21 phones; TPS-visible 17→38); processing-status linked to committed batches (`done=123`). **Category readiness:** mobile READY, air_conditioner PARTIALLY_READY (no `write_ac_batch`), others NOT_READY — no category fabricated. **AC investigated (ADR-020):** gap report + `write_ac_batch` spec produced; batch NOT executed — evidence shows AC is structurally single-store (955 اكسترا vs 45 المنيع, 0 ≥2-store corroboration in the dry-run; only 3 corroborated ACs exist), so the Phase-6 precision gate is unmet. **Multi-store AC matcher built (ADR-021):** `ac-matcher-v1-dry.ts` (balanced Extra+Almanea, store_id-based, ≤500); genuine balanced dry-run yields **0 ≥2-store corroboration** — a store-coverage gap (Extra & Almanea stock different AC brands), so **no AC write executed**. **Remaining E6 (see `docs/HANDOFF-E6.md`):** build `write_ac_batch` as tested infra, close AC store-coverage/parser gap, automated TPS tests, scheduler+overlap wiring. |
