...
| **E7** | Canonical linkage on ingestion | ✅ **Complete (ADR-044)** — /api/cron/dispatch fires a throttled bounded progressive sweep every ~15 min; newly-ingested observations are linked + corroborated continuously (durable cursor). |
...
| **E14** | Owned search index authority cutover | 🟢 **Core SHIPPED & production-verified (ADR-040)** — hybrid authority live: `/api/v1/tps/search` returns Layer 1 comparison (94 corroborated) + Layer 2 resolved-single discovery (labelled, measured exits), `meta.authority=hybrid`; 0 false-comparison; catalog preserved (single-store products incl. laptops surface as resolved-single). Owned index 394 (94 comparison + 300 resolved-single). **Remaining:** Layer 3 raw long-tail + shadow/canary before SOLE-authority cutover. |
| **E15** | Legacy retirement (System B, VPS) | ✅ **OPERATIONALLY COMPLETE — production-verified (ADR-042)**: zero operational dependency on the legacy system; every subsystem (web/TPS search/ingestion/cron/recommendations/measured-exits/Algolia/identity/progressive/mobile) runs on System A; tawveeri.com bundle references System B 0×. Legacy ffps/etlaq.sa still physically exist as THIRD-PARTY infra (not in founder account, not production entry point) — teardown is the owner's discretion, not a Tawveeri dependency. E15 CLOSED. |
| **E15.5** | Post-E15 bridge → Knowledge-Graph/Agent track | 🟢 **Gate assessed 2026-07-25 (ADR-093): PASS (technical) · PARTIAL (real-user).** `docs/E15-5-GATE-REPORT.md` + `npm run tps:gate-audit` (reproducible evidence): Stage-1 Decision Agent LIVE + **publicly surfaced** (`/advisor`), **295 real ≥2-store comparisons (254 `/go`-reachable)**, 6+ live categories, 6/8 stores fresh, honest single-store labelling. **Visible Trust experience** shipped (Arabic evidence panel: verified fact / inferred / unknown / insufficient). **Real-vs-test instrumentation** live (`usage_events` + `/api/events` + `npm run tps:usage`). Residuals gating full closure: **0 real users (launch 1-Aug)**, noon/swsg staleness, 2,133 unresolved normalized identities. **E16 is NOT opened formally until the real-user loop + instrumentation are production-verified with real traffic.** Earlier: Coverage Matrix, E7 continuous linkage, Saudi Agent Benchmark v1 (8/8), UCP adapter, Merchant Twin, Revenue-Neutrality verified (ADR-044). |
| **E16** | Contracts & documentation alignment (regenerate types) | Continuous |
| **Phase 2** | Decision Intelligence + universal merchant/monetization platform (active engineering milestone, Founder directive 2026-07-24/25) | 🟢 **IN PROGRESS — shipping.** **Automation (ADR-078)**: intelligence chain self-refreshes hourly on Railway, production-verified (IPv4 pooler + instrumentation-spawn + guard fixes). **Identity**: mobile 86→98.3% (ADR-080/081, storage-optional NO_STORAGE canonical); appliance identity-normalization measured & closed FAIL (ADR-083 — merchant-overlap-bound, not identity). **Merchant framework (ADR-082/085/086)**: recurring shaker/samsung_ksa ingestion; a generic pluggable **Provider Framework** (sourcing × monetization adapters, feature flags); Amazon affiliate reference LIVE (`/go` → tag=tawveeri-21 + ascsubtag conversion sub-id); a **credential-free WooCommerce Store-API feed adapter** wired end-to-end (`tps:ingest-provider`), reusable for any Woo/Salla shop. **Trust & Evidence Engine (ADR-087)**: deterministic, transparent, evidence-cited Trust Score (6 weighted factors, corroboration-dominant, unknown→conservative) unified across the Decision Agent, TPS search, recommendations, and UCP feed — replacing the ad-hoc confidence heuristic. **Security**: `/api/debug/scheduler` gated. **Landscape (evidence)**: comparison growth is now merchant-DATA-ACCESS-bound (high-overlap retailers block scraping; feeds/affiliate credentials are Founder boundaries the framework plugs in config-only). **Next**: enrich trust with freshness/discount everywhere; knowledge/compatibility graph; agent reasoning/planning over the deterministic core; more feed connector types (CSV/XML/Merchant Center) as the universal onboarding framework. |
...
> **Post-E15 (2026–2040):** the strategic evolution to a Saudi Commerce Intelligence OS — Knowledge Graph + Product DNA, Two-Stage Agent (Stage-1 early), UCP-first-but-protocol-neutral, Merchant Independence, ranking-blind Revenue Graph — is specified in `docs/POST-E15-STRATEGY-2026-2040.md` (+ `docs/POST-E15-GLOBAL-RESEARCH-AUDIT.md`, ADR-043). Awaiting founder approval before broad implementation.
...
| **camera** | ✅ **Live — production-verified** | **3 canonicals** (bounded run) | 6th live category; brand|model(+variant)|config; R50≠R50 V, body≠kit; price-band dropped double-kit (ADR-038) |
