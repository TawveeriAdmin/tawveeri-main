...
| **E6** | TPS pipeline automation + **canonical-quality audit** (resolves the identity-invariant debt) | **Taxonomy remediation + first bounded pipeline batch SHIPPED & PRODUCTION-VERIFIED (ADR-011/013/014/015).** 788 canonical rows corrected; pipeline entry point found (TS matcher over HTTPS + `write_mobile_batch`), bounded to ≤500 obs, dry-run-first; production batch wrote 17 corroborated mobile canonicals (incl. iPhone 15, closing the E8 gap), verified, no regressions. **Follow-on:** projection rebuild for user-facing surfacing, `mobile`/`smartphone` naming harmonization, and scaling the bounded batch across categories/backlog. |
