...
| **E15.5** | Post-E15 bridge → Knowledge-Graph/Agent track | 🟢 **E15.5 in progress (ADR-044)** — Coverage Matrix live (docs/COVERAGE-MATRIX.md); E7 continuous linkage closed; **Stage-1 Decision Agent v0 LIVE + production-verified** (POST /api/v1/agent/decide — deterministic, ranking-blind, total-cost, Product DNA; flagship AC journey); Saudi Agent Benchmark v1 (8/8). Decision Agent covers ac/tv/tablet/mobile + free-text parser; UCP protocol adapter/feed; Merchant Twin; Revenue-Neutrality VERIFIED (docs/AGENT-API.md). Next: audio/camera fallback tuning + Consumer Twin (PDPL-gated). |
