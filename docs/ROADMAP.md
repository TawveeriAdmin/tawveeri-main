...
| **E15** | Legacy retirement (System B, VPS) | ⛔ **BLOCKED on decommission execution only (ADR-041)** — readiness VERIFIED by production evidence: no runtime dependency on System B (web bundle/API/mobile eas.json/cron all System A), no required data (2 zero-activity users; superseded catalog), deleting B breaks no subsystem. Legacy services still running (etlaq.sa health=200). Sole blocker: deleting the Supabase project + shutting the VPS needs owner platform credentials — DEMONSTRATED ABSENT (all tokens absent; anon DELETE→401). External; engineering cannot eliminate. Prior archive-credential blocker WITHDRAWN (no archive required). |
