...
### ADR-281 — Post-freeze measurement gap fix: Decision Card v1 events wired on Path-2 (SmartPickCard), size_mismatch persisted, ADR-271 zero-state fallback given its own event · Accepted (2026-08-31)

**Context.** Pulling the post-freeze (2026-08-24→08-31) production numbers for Decision Card v1 (ADR-270) and the zero-state fallback (ADR-271) surfaced three real instrumentation gaps, not design or ranking defects: (1) `recommendation_accept` and `alternative_view` showed **0 events all-time** — Path-2's plain-search decision card (`smart-pick-card.tsx`) had zero `track()` calls of any kind, while `go_click`-by-source data showed most real exits come through Path-2, not Path-1's advisor card, where the four Decision Card v1 events (ADR-270 §D) were wired from the start; (2) `size_mismatch` (ADR-270 ruling B1) is computed server-side (`decide/route.ts`, `search/route.ts`'s `buildDecisionLayer`) but never persisted to `usage_events` — mismatch-vs-clean-match rate was unmeasurable; (3) ADR-271's "closest options" zero-state fallback is set into client React state but never logged, so its trigger rate was indistinguishable from the broader `no_answer` catch-all. Founder-scoped explicitly as tracking-only, same-week discipline as every other change: `decision-engine.ts`/`route-query.ts`/`evidence-engine.ts` stay frozen, no ranking/matching/eligibility logic touched anywhere.

**Decision — wire 3 of Path-1's 4 events on Path-2; document the 4th as a real gap instead of fabricating it.** `recommendation_accept` (primary CTA click) and `return_to_decision` (the same shared `sessionStorage` helper, `return-to-decision.ts`, Path-1 already used — reused verbatim, not reimplemented) map cleanly onto Path-2 with no UI change. `alternative_view` maps to the existing "Compare N stores" click (`meta.via: 'compare_link'`) — the closest real affordance to "viewed an alternative" this simpler card has; tagged distinctly since it is store-level (this SAME product at other merchants), not product-level like Path-1's `chosen_over` (a DIFFERENT product). `evidence_expand` is deliberately left **unwired**: Path-2's card has no expand/evidence panel at all, and firing an "expand" event on a click that expands nothing would fabricate a signal the Constitution's "unknown beats incorrect" rules out. Needed one small plumbing addition to carry these: a `canonical_id` field on the decision-card payload (`search/route.ts`, `tps_identity_key` when TPS-linked else the storefront `product_id`) purely for event correlation — never read by ranking or matching.

**Decision — `mismatch` rides as a boolean field on the existing `advisor_result`/`results` events, not a new event type.** Added to all 4 call sites that already fire those events (search page ×3, product page ×1) — per the same-event-carries-a-field instruction, since a new event type wasn't needed to answer the question.

**Decision — `closest_options_view` IS a new, narrow event type.** Fires only when the ADR-271 fallback actually surfaced candidates (`closestOptions.length > 0`), deliberately distinct from `no_answer` (which fires for ANY zero-result cause and says nothing about whether the fallback found something to show).

**Verification.** `tsc --noEmit` diffed against a pre-change baseline: zero new errors anywhere (only line-number shifts from added lines). Full suite: 161/161 suites, 2612/2612 tests. Live-verified against the running dev server (points at the same production Supabase) under `?test=1` isolation, via an intercepted-`fetch` harness rather than trusting the CDP network log alone (which reported spurious 503s for `keepalive` requests that a direct in-page `fetch()` proved were actually 204s): confirmed `mismatch: true` vs `mismatch: false` firing correctly on a real TV size-mismatch query ("tv 100 inch") vs a real clean-match query ("tv 43 inch"); confirmed `alternative_view`/`recommendation_accept` fire with the correct `canonical_id` and `meta` on real clicks; confirmed `return_to_decision`'s `sessionStorage` mark (`tawveeri:decision_card_seen:<canonical_id>`) writes correctly on mount; confirmed `closest_options_view` fires with the correct candidate count alongside — not instead of — `no_answer` when a forced `max_price` produced a genuine zero-result/fallback response from `selectClosestOptions`. A couple of real-tagged `search`/`results` rows were incidentally written to production before `?test=1` isolation was set up for this session's own manual QA — ordinary verification noise, not a data-quality concern for anyone reading the metrics.
