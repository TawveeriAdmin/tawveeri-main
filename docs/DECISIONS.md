...
*Residual, smaller, NOT fixed — logged honestly rather than chased for diminishing returns:* two TVs mentioning "AC/DC-12V" power specs and a router mentioning "802.11ac" Wi-Fi still surface in the "مكيف" lowest-price sort (positions #2–3, 865–1149 SAR, still above the cheapest genuine AC at 1065 SAR) via the SAME mechanism but on "ac" specifically — deliberately NOT stoplisted, since "ac" is a genuinely necessary term for AC search recall (many real listings are literally titled "Split AC"/"Window AC"); removing it would trade a smaller, already-much-reduced leak for worse recall on genuine AC queries. This is a materially smaller version of bug #7 (3 items, not 15+; the cheapest genuine AC is now position #4, not somewhere past a full page of junk) and is left as a known, scoped gap rather than presented as fully solved.
