...
Also found, NOT fixed this addendum — a genuinely different, harder mechanism: "خلاط" (blender) surfaces a "Blender Bottle"-brand protein shaker and its travel-cup accessories, since the BRAND NAME itself literally contains the word "blender". This is a brand-name collision, not an accessory-hint gap or a generic-token leak — none of this session's established fix patterns directly apply. Logged honestly as a known, unresolved finding for a future pass rather than silently dropped or force-fit into an ill-suited fix. Full test suite 1661→1663, zero regressions.

**Cumulative cross-category sweep result, all four passes this session:** مكيف/شاشة/راوتر/مكنسة/غسالة/ساعة/فرن/مكواة/مايكروويف/قلاية — 10 categories with real, distinct leaks found and fixed, spanning 5 root-cause mechanisms. تلفزيون/ثلاجة/تابلت/كاميرا/سماعة/جوال/لابتوب/نشافة — 8 categories checked clean. خلاط — found contaminated (brand-name collision), left unfixed and explicitly logged.
