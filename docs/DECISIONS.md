...
- Combined with ADR-013's AC fix: **731 canonical rows corrected** this session; `accessories` 1,079 → **387**; enum duplicates eliminated. Final distribution: smartphone 248, accessories 387, air_conditioner 414, smartwatch 66, appliance 439, tablet 13, other 305, audio 220, tv 25, monitor 35, camera 1, laptop 15 (2,168 total, all active).
**Pipeline-automation half — BLOCKED (Founder Approval boundary):** automating normalize→resolve→canonicalize over the **129,715 pending** `raw_observations` cannot be done safely: the pipeline's DB functions have **no definitions in the repo** (built out-of-band), `SUPABASE_DB_URL` is absent so their signatures/bounds can't be inspected, and the RPC existence-probe returns false positives. Invoking them blindly would write canonical products / matches / price_history across the **entire backlog** — a large, hard-to-reverse data mutation with no confirmable bound. Per the Executive Prompt this is a "destructive or irreversible data change" boundary and stops for founder input.
