...
- **Ongoing coverage SHIPPED + verified:** `POST /api/cron/tps-progressive` (auth CRON_SECRET, ≤500/unit, up to 10 units/call) runs the durable-cursor sweep on newly-ingested observations. Verified live (401 gate; authorized run scanned 468, corroborated tv+7/tablet+6). **To automate:** add a `scraping_schedules` row or PM2 tick calling it (like `/api/cron/tps-batch`); currently invoke-on-demand.
- **E11 mobile client hybrid-ready + type-verified:** `mobile/src/lib/api/platform.ts` matches the E14 hybrid response (results + discovery + meta; measured-exit offer_ids). **Remaining E11 (build-gated):** migrate deals/index/search/product/store screens off direct `products`/`product_stores` reads to `platformApi` + render decision objects + route exits via `openMeasuredExit`; then `expo` type-check/build + app-store RC. Needs an Expo/Xcode build env (not available in the current session).
