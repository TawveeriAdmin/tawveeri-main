...
|-------|-------|:-----:|:-----------:|:-------:|:--------:|:----------:|
| **Phase 1** | Growth & Adoption | 1–5 | 22 | 5 | 0 | **91%** |
...
The core platform is largely built. Users can register, search, compare prices, view products, save wishlists, and browse stores. The main gaps are in search filtering depth and cron scraper completion.
...
| 6 | Phone-based password reset | 🟡 | Email reset works. Phone reset UI says "coming soon" in `forgot-password/page.tsx` |

---

### Week 3 — Search & Filtering `65%`

| # | Task | Status | Evidence |
|---|------|:------:|----------|
| 1 | Product search by category (TV, Laptop, Smartphone) | 🟡 | Category chips + select exist in UI (`search-bar.tsx`), but category doesn't filter scraped results on the backend |
| 2 | Filtering by brand, model | ✅ | Dynamic brand checkboxes from DB in `src/components/search/filter-sidebar.tsx` |
| 3 | Filtering by RAM, storage, size, resolution, color | 🟡 | `specifications` field is captured in schema; **no filter UI** built for these attributes |
| 4 | Dynamic filters (based on category) | 🟡 | Only brands are category-dynamic; all other filters are static |
...
| # | Task | Status | Evidence |
|---|------|:------:|----------|
| 1 | Integration with 5 stores (Extra, Jarir, Almanea, Noon, Amazon.sa) | 🟡 | All 5 stores configured. **Search scrapers** work for 4 stores. **Cron scrapers**: only Jarir is fully implemented; Amazon, Noon, Extra, Almanea are stubs that throw "not yet implemented". |
...
| Gap | Impact | Effort |
|-----|--------|--------|
| Complete cron scrapers for Amazon, Noon, Extra, Almanea | High — data freshness for automated price updates | Medium |
| Add spec-based filter UI (RAM, storage, screen size, color) | High — core differentiator for electronics platform | Medium |
| Make category selection actually filter results | Medium — currently decorative only | Low |
| Phone-based password reset | Low — email reset works | Low |
...
| 4 | **Complete cron scrapers** (Amazon, Noon, Extra) | High | Medium | Price data freshness for automated alerts and history. |
| 5 | **Push notifications** — service worker + Web Push API | High | Medium | Settings UI exists, backend missing. |
| 6 | **Premium store upgrade flow** — payment integration | High | High | DB fields exist; needs payment gateway. |
| 7 | **AI recommendations** | Medium | High | Could start with simple collaborative filtering before full ML. |
| 8 | **Monitoring** — Sentry / Datadog / UptimeRobot | Medium | Low | Required for production reliability. |
| 9 | **Accessibility audit** — WCAG 2.1 AA | Medium | Medium | Radix helps; needs ARIA labels, landmarks, contrast fixes. |
| 10 | **Loyalty / Cashback program** | Medium | High | Zero foundation — needs design + full implementation. |
| 11 | **Hijri calendar + Arabic numerals** | Low | Medium | Saudi-specific localization. |
| 12 | **Mobile app** (React Native / Expo) | Low (short-term) | Very High | Entire new codebase. Consider after web is production-ready. |
