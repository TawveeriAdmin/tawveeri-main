...
**The clarification question is built** (`306a8b4`, ADR-153) — scoped *inside* P2-8, because it is a branch of the routing decision, not a new unit. It asks only when the engine proves at both ends of the offered range that the answer changes the recommendation; it never asks for a field the query already supplied; and it is not a gate (labelled skip, answer already on screen). It also exposed and fixed a parser defect: **`\d` never matched Arabic-Indic digits**, so «لغرفه ٤٠ متر» lost its room size, its budget and its storage size silently — the third occurrence of that trap in this codebase.

**Comparison-intent routing is built** (`1b8113b`, ADR-154) — the fourth and last §UNIFIED SEARCH routing branch, **its own unit, not P2-8**. A comparison is offered only where `getComparison()` — the compare page's own loader — proves it will render ≥2 approved-retailer offers. Pair requests never route, because no page can fulfil them. Measured: **only 15.1% of canonicals (761/5,054) have ≥2 retailers**, so "no comparison available" is the common answer. It also fixed a retrieval defect: comparison queries returned **0 identity-bearing results** because the marker words were matched against product text.

**§UNIFIED SEARCH is now COMPLETE.** Phase 2 remains **open** on four blocked units (P2-4 traffic, P2-5 F7, the retailer-count amendment, DEBT-1) — see `HANDOVER.md` for each block and its unblocking condition.

**The honest verdict, recorded in ADR-152 rather than glossed:** one capability at the *surface* — entry point, routing, answer, disclosure — and **two engines underneath**. `/api/search` and `/api/v1/agent/decide` remain separate systems, and the engine advises on **17 categories** while everything else falls to retrieval. **One** named UNIFIED SEARCH behaviour is still unbuilt: comparison-intent routing («قارن بين X و Y» falls to retrieval), which needs a new destination and a comparison-generation capability — **its own unit, not P2-8**. **No constitutional amendment is proposed:** nothing measured shows the principle cannot be achieved.
