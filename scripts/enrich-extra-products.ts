/**
 * Detail-page enrichment pass for Extra products.
 *
 * The full-coverage seed run scrapes Extra products from listing cards only,
 * leaving description_en/ar=NULL, specifications={}, image_urls=[1], and no
 * merchant rating. This script walks each product's detail page via the
 * ExtraScraper's scrapeProductPage (invoked through updateProductPrice) and
 ...
 *
 * Usage:
 *   # fill the next 6000 products
 *   ENRICH_LIMIT=6000 npx tsx scripts/enrich-extra-products.ts
 *
 *   # restrict to specific categories
 *   ENRICH_CATEGORIES=kitchen,smart_home npx tsx scripts/enrich-extra-products.ts
 *
 *   # start over
 *   ENRICH_RESET_STATE=true npx tsx scripts/enrich-extra-products.ts
 *
 * Rate-limit safety: shares one ExtraScraper instance (and one RateLimiter)
...
async function main(): Promise<void> {
  ...
  // Products are enriched sequentially — the ExtraScraper rate limiter is
  // process-wide, and detail pages require Puppeteer (one browser per scraper
  // instance), so parallel fan-out would just serialize against both anyway.
  ...
  const STATE_FILE = process.env.ENRICH_STATE_FILE
    || path.join(__dirname, '..', '.scrape-state', 'extra-enrich.json');
  ...
  // when the scraper returns null repeatedly (likely Puppeteer navigation
  // failure or anti-bot page, not just a transient 5xx).
  ...
  // continuing. Tune up if Extra is particularly grumpy.
  ...
  console.log(
    `[${stamp()}] enrich-extra-products starting — limit=${LIMIT} only_missing=${ONLY_MISSING} resume=${RESUME}`
  ...
  try {
    while (!done && processedNow < LIMIT) {
      const rows = await fetchBatch(supabase, extraStoreId, {
        cursor,
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (!scraped) {
            ...
            // nulls are piling up (likely Puppeteer navigation failure /
            // anti-bot page wall).
            failed++;
