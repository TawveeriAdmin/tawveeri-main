...
const VERSION = 'tawveeri-cron-2026-06-08-v6-with-slug';
...
function firstStr(val: unknown): string {
 ...
 if (typeof val === 'object') {
   for (const v of Object.values(val as Record<string, unknown>)) { const s = firstStr(v); if (s) return s; }
 }
 return '';
}
...
export async function GET(request: NextRequest) {
 ...
 if (!slug) {
   return json({ status: 'ok', stores: STORES.map(s => s.slug) });
 }
