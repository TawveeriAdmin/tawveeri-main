...
async function saveProducts(products: any[]): Promise<any> {
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    if (selectError) { errors.push(selectError); continue; }
    ...
    if (error) { errors.push(error); console.error('[Insert Error]', error); continue; }
    saved += data?.length || newRows.length;
  }

  return { saved, errors, totalRows: rows.length, sampleRow: rows[0] || null };
}
