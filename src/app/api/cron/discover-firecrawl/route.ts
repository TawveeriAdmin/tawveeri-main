...
const VERSION = 'tawveeri-cron-2026-06-09-v9-final';
...
function firstStr(val: unknown): string {
 ...
 if (Array.isArray(val)) {
   for (const v of val) { const s = firstStr(v); if (s) return s; }
   return '';
 }
 if (typeof val === 'object') {
   for (const v of Object.values(val as Record<string, unknown>)) {
     const s = firstStr(v); if (s) return s;
   }
 }
 return '';
}
...
async function saveProducts(products: any[]): Promise<any> {
 ...
 for (const p of rows) {
   try {
     ...
     if (selectErr) { errors.push({ step: 'select_product', error: selectErr }); continue; }
     ...
     const { data: existingStore, error: storeSelectErr } = await sb
       .from('product_stores')
       .select('id')
     ...
     if (storeSelectErr) { errors.push({ step: 'select_store', error: storeSelectErr }); continue; }

     if (existingStore?.id) {
       await sb.from('product_stores').update(storeRow).eq('id', existingStore.id);
     } else {
       const { error: insertStoreErr } = await sb.from('product_stores').insert(storeRow);
       if (insertStoreErr) { errors.push({ step: 'insert_store', error: insertStoreErr }); continue; }
     }
 ...
 return { saved: savedProducts, savedProducts, savedStores, errors: errors.length ? errors.slice(0, 5) : undefined, totalRows: rows.length };
}
