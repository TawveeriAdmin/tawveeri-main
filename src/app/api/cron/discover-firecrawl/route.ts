...
// ─── STORES — معرّفة أولاً في Global Scope ────────────────
const STORES = [
...
export async function GET(request: NextRequest) {
  ...
  if (!slug) return NextResponse.json({ status: 'ok', stores: storeList.map(s => s.slug) });
  const store = storeList.find(s => s.slug === slug);
