...
const VERSION = 'tawveeri-cron-2026-06-09-v8-real-schema';
...
async function saveProducts(products: any[]): Promise<any> {
  ...
  for (const p of rows) {
    try {
      ...
      if (selectErr) {
        errors.push({ step: 'select_product', error: selectErr });
        continue;
      }
      ...
      if (storeSelectErr) {
        errors.push({ step: 'select_store', error: storeSelectErr });
        continue;
      }

      if (existingStore?.id) {
        const { error: updateStoreErr } = await sb
          .from('product_stores')
          .update(storeRow)
          .eq('id', existingStore.id);

        if (updateStoreErr) {
          errors.push({ step: 'update_store', error: updateStoreErr });
          continue;
        }
      } else {
        const { error: insertStoreErr } = await sb
          .from('product_stores')
          .insert(storeRow);

        if (insertStoreErr) {
          errors.push({ step: 'insert_store', error: insertStoreErr });
          continue;
        }
      }
...
export async function GET(request: NextRequest) {
  ...
  if (!slug) {
    return json({
      ...
      testUrl: '/api/cron/discover-firecrawl?store_slug=almanea-direct&sync=1&pages=1&limit=20',
    });
  }
